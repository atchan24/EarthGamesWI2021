...
### THIS IS FOR PLAYER 4: Beat

signal player4lose(score)
signal player4add(score)
...
func _process(delta):
	...
	if moving:
		...
		if pos.length() < 1 :
			...
			# This is what stops th eplayer going around the board!!
#			var temp = current + 1
#			if temp >= spaces.size():
...
func update_values(s):
	...
	if s > 0:
		emit_signal("player4add", s)
	else:
		emit_signal("player4lose", s)
